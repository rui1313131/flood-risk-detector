...
## 高知県安芸市港町二丁目（ユーザ環境の PowerShell から実行）

太平洋に面した小規模な漁港集落。山地（背後）と湾岸低地が同フレーム内にあり、検出が地形に従うかの良いストレステスト。

```powershell
chcp 65001 | Out-Null
$env:PYTHONIOENCODING="utf-8"
python run_site.py 33.501860 133.91322 --label "高知県安芸市港町二丁目"
```

| 項目 | 値 |
|---|---|
| 中心座標 | 33.502°N, 133.913°E |
| 標高範囲 | -0.2 〜 250.3 m（湾岸低地 + 背後の山地） |
| 検出窪地 | 87 件 |
| 対象窪地（住宅含む & 標高 ≤ 10 m） | 10 件 |
| 建物総数 | 258 棟 |
| 浸水リスク建物 | 40 棟 |
| FillSink バックエンド | pure |
| DEM レイヤ | GSI dem5a_png z15 |
...
成果物:
- [`elevation_gsi_classic.png`](kochi_aki/elevation_gsi_classic.png)
- [`map_overlay_with_sinks.png`](kochi_aki/map_overlay_with_sinks.png) — 港町住宅地と港湾エリアの低地に窪地が集中、背後の山地には窪地ゼロ
- [`report.txt`](kochi_aki/report.txt)
- [`at_risk_buildings.csv`](kochi_aki/at_risk_buildings.csv)
